...
public class categorie_qestionnaireService {
    ...
    public categorie_questionnaire get(int id)
    {
      categorie_questionnaire c =new categorie_questionnaire();
        ...
        try {
            ...
            ResultSet res=st.executeQuery("SELECT *  FROM  categorie_questionnaire where ID="+id);
            while (res.next()){
    ...
    public boolean update(categorie_questionnaire u , int id) throws SQLException {
      ...
      public boolean incrementer( int id) throws SQLException {
        try{
            Connection cnx = DataSource.getInstance().getCnx();
            PreparedStatement preparedStmt = cnx.prepareStatement("UPDATE categorie_questionnaire set nbr_qst= nbr_qst+1  where id = ? ");
