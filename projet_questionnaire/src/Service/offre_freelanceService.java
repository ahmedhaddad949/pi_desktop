...
public class offre_freelanceService {
    ...
    private static final  String SEARCH_QUERY = "SELECT *  FROM  offre_freelance WHERE UPPER(ID) LIKE ? OR UPPER(titre_offre_fr) LIKE ? OR  UPPER(descriptionfr) LIKE ? OR  UPPER(entreprisefr) LIKE ? OR  UPPER(recomponse) LIKE ?  OR  UPPER(date_expiration) LIKE ? OR  UPPER(etat_offre) LIKE ? OR  UPPER(categorie_freelance_id) LIKE ?";
 public offre_freelance get_offre_freelance_affichage(int i) {
        offre_freelance c = new offre_freelance();
        int nombre = 0;
      String requete = "SELECT *  FROM  offre_freelance "  ;
...
public int nombre() {
        ...
        int i = 0;
        String requete = "SELECT COUNT(*) as nbr from offre_freelance  ";
