...
import static Service.formationService.printSQLException;
...
public class ReclamationService {
        ...
        public boolean add(reclamation u) throws SQLException {
            try{
                ...
                preparedStmt.setDate (5, new java.sql.Date(new java.util.Date().getTime()));
        ...
        public ObservableList<reclamation> Searsh(String cas)

    {
        ...
        try {
            Connection cnx = DataSource.getInstance().getCnx();
            st=cnx.createStatement();
            ResultSet res=st.executeQuery("SELECT * FROM `reclamation`  where  ( categorie_id LIKE '%" + cas + "%'or  user_id LIKE '%" + cas + "%' or  titre LIKE '%" + cas + "%' or  titre LIKE '%" +  cas + "%' or  date LIKE '%" + cas + "%' )");
            while (res.next()){
                reclamation c =new reclamation();
                 c.setId(res.getInt("ID"));
                    c.setCategorie_id(res.getInt("categorie_id"));
